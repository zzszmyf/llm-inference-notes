...
1. [本章目标](#1-本章目标)
2. [信息论三件套：bit、熵、率失真](#2-信息论三件套bit熵率失真)
3. [整数表示](#3-整数表示)
4. [定点数与 scale 的世界](#4-定点数与-scale-的世界)
5. [IEEE 754 浮点数](#5-ieee-754-浮点数)
6. [舍入（Rounding）](#6-舍入rounding)
7. [截断（Truncation）与饱和（Clamp）](#7-截断truncation与饱和clamp)
8. [计算机组成视角：为什么 bit 少就快](#8-计算机组成视角为什么-bit-少就快)
9. [预备知识$\to $量化概念映射表](#9-预备知识--量化概念映射表)
10. [习题与解答](#10-习题与解答)
11. [延伸阅读](#11-延伸阅读)
...
5. 下一篇：**[02 量化问题的形式化与均匀量化理论](./LLM量化精读笔记-02-量化问题形式化与均匀量化理论.md)**——用本章的编码与误差工具，正式建立量化的数学框架。
